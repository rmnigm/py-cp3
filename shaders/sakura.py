...
from utils import plot, pal, length, fract
...
def render(t: ti.f32, frame: ti.int32):
    for frag_coord in ti.grouped(pixels):
        ...
        col = ((ts.vec3(ts.smoothstep(fract(d), fract(d) + -0.200, 0.160))
                - ts.vec3(ts.smoothstep(fract(d), fract(d) + -1.184, 0.160)))
               * pal(f,
                     ts.vec3(0.725, 0.475, 0.440), ts.vec3(0.605, 0.587, 0.007),
                     ts.vec3(1.0, 1.0, 1.0), ts.vec3(0.310, 0.410, 0.154))
               )
